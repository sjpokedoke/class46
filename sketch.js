    ...
    if(pumpkinsr.x>500&&pumpkinsr.x<800){
      for(var a=0; a<pumpkingroupr.length; a++){
           if(pumpkingroupr.isTouching(luna)){
            luna.changeAnimation("lunahurt", lunahurt)
            hurtsound.play();
...
//Left to do: add the score; the drwaing function; sounds for destroying, luna change image;  add the restart functoin; debug and fix up pumpkins; add heart function